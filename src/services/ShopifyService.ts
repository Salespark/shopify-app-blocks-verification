...
class ShopifyService {
    ...
    public static shopifyThemes = async (shop: string, accessToken: string) => {
        ...
        const {data: {themes}} = await HttpClient.shopifyGetRequest(themeUrl, accessToken);

        return themes
    }

    public static shopifyThemeAssets = async (shop: string, accessToken: string, publishedTheme: any, query: any = {}) => {
        ...
        if (Object.keys(query).length > 0) {
            ...
            themeUrl += `?${qs}`
            const {data: {asset}} = await HttpClient.shopifyGetRequest(themeUrl, accessToken);
            return asset
        }
        const {data: {assets}} = await HttpClient.shopifyGetRequest(themeUrl, accessToken);
        return assets;
    };

    public static findTemplateMainSections = async (shop: string, accessToken: string, publishedTheme: any, assets: Array<any>, templateJSONFiles: Array<any>) => {
        return (await Promise.all(templateJSONFiles.map(async (file, index) => {
            const asset = await ShopifyService.shopifyThemeAssets(shop, accessToken, publishedTheme, {"asset[key]": file.key});
            const json = JSON.parse(asset.value)
            const sections: Array<any> = Object.entries(json.sections);
            const main: any = sections.find(([id, section]) => id === 'main' || section.type.startsWith("main-"))
            if (main) {
    ...
    public static findSectionsWithAppBlock = async (shop: string, accessToken: string, publishedTheme: any, assets: Array<any>, templateMainSections: Array<any>) => {
        return (await Promise.all(templateMainSections.map(async (file, index) => {
    ...
    public static verifyAppBlocks = async (request: any) => {
        ...
        const templateMainSections = await ShopifyService.findTemplateMainSections(shop, accessToken, publishedTheme, assets, templateJSONFiles);
        const sectionsWithAppBlock = await ShopifyService.findSectionsWithAppBlock(shop, accessToken, publishedTheme, assets, templateMainSections);
